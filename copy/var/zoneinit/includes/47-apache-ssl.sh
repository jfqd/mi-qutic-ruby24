...
if mdata-get server_ssl 1>/dev/null 2>&1; then
	(
	umask 0077
	mdata-get server_ssl > "${SSL_HOME}/server.pem"
	# Split files for kumquat usage
	openssl pkey -in "${SSL_HOME}/server.pem" -out "${SSL_HOME}/server.key"
	openssl crl2pkcs7 -nocrl -certfile "${SSL_HOME}/server.pem" | \
		openssl pkcs7 -print_certs -out "${SSL_HOME}/server.crt"
	)
	...
	ln -sf ${LE_LIVE}/fullchain.pem ${SSL_HOME}/server.crt
	ln -sf ${LE_LIVE}/privkey.pem ${SSL_HOME}/server.key
	...
	rm -f ${SSL_HOME}/server.csr ${SSL_HOME}/server.pem
fi
